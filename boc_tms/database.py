...
database.py
------------
SQLite data-access layer for the BOC WPS Transport Management System.

Responsibilities:
  * Schema creation (drivers, vehicles, departments, appointments, users)
  * Seed data for first-run demo/interactivity
  * CRUD helpers returning pandas DataFrames (easy to render in Streamlit)
  * Booking-conflict validation (no driver/vehicle double-booking)

Every write operation (INSERT/UPDATE) calls `excel_sync.trigger_backup()`
at the end so the Excel mirror stays in sync without blocking the caller.
"""
...
# --------------------------------------------------------------------------
# Trip requests (Department -> Transport Officer workflow)
# --------------------------------------------------------------------------
...
# --------------------------------------------------------------------------
# Departments management + linked-login creation
# --------------------------------------------------------------------------
